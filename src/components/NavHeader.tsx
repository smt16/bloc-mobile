  ...
  return (
    <View className="flex-row items-center gap-md py-sm">
      <Pressable
        ...
        className="h-[42px] w-[42px] items-center justify-center rounded-pill border border-border bg-surface active:opacity-60"
        hitSlop={8}
      >
        <Icon name={icon} size={22} color={colors.text} />
      </Pressable>
      {title ? (
        <Text className="flex-1 text-h2 text-text" numberOfLines={1}>
          {title}
        </Text>
      ) : (
